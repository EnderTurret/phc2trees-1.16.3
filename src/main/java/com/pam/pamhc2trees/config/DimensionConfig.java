...
public class DimensionConfig {
	...
	public static void init(ForgeConfigSpec.Builder config) {
		whitelist = config.comment("Enter a dimension id to whitelist feature generation").defineList("white_dim",
				Arrays.asList("minecraft:overworld"), val -> val instanceof String str && ResourceLocation.tryParse(str) != null);

		blacklist = config.comment("Enter a dimension id to blacklist feature generation").defineList("black_dim",
				Arrays.asList("minecraft:the_end", "minecraft:the_nether"), val -> val instanceof String str && ResourceLocation.tryParse(str) != null);
	}
