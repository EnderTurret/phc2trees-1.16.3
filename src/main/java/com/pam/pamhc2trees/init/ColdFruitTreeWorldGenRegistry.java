...
public class ColdFruitTreeWorldGenRegistry {
	...
	public static void addToBiomes(BiomeLoadingEvent evt) {
		Set<BiomeDictionary.Type> types = BiomeDictionary.getTypes(RegistryKey.create(Registry.BIOME_REGISTRY, evt.getName()));
		//maple
		if (WorldGenRegistry.maple_worldgen != null) {
			if (types.contains(BiomeDictionary.Type.CONIFEROUS)) {
				evt.getGeneration().addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
